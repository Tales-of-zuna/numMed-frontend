    ...
    <v-row
      ><v-col cols="12"
        ><v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn to="/tools" @click="deleteOne()" color="error">Устгах</v-btn>

            <v-btn to="/tools" color="teal" dark @click="update()">
              Хадгалах
            </v-btn>
          </v-card-actions>
        </v-card></v-col
...
export default {
  ...
  methods: {
    ...
    deleteOne() {
      let response = this.$axios.delete("/tools/" + this.$route.params.id);
      if (response.status == 200) {
        this.product = response.data;
      }
    },
  ...
  async fetch() {
    let response = await this.$axios.get("/tools/" + this.$route.params.id);
    if (response.status == 200) {
      this.product = response.data;
    }
  },
